# Source https://github.com/arminmokri/v2ray2json
import argparse
import base64
import json
...
class InboundBean:
    class SniffingBean:
        ...
        def __init__(self, enabled: bool, destOverride: list[str], metadataOnly: bool) -> None:
    ...
    class StreamSettingsBean:
        class TcpSettingsBean:
            class HeaderBean:
                ...
                def __init__(self, type: str = "none", request: RequestBean = None) -> None:
                    self.type = type
            ...
            if self.network == "tcp":
                ...
                if headerType == HTTP:
                    ...
                    if host != "" or path != "":
                        ...
                        requestObj.headers.Host = "" if host == None else host.split(",")
                        requestObj.path = "" if path == None else path.split(",")
                        tcpSetting.header.request = requestObj
                        sni = requestObj.headers.Host[0] if len(requestObj.headers.Host) > 0 else sni
                else:
...
def get_outbound_trojan():
    outbound = OutboundBean(
        ...
        settings=OutboundBean.OutSettingsBean(servers=[OutboundBean.OutSettingsBean.ServersBean()]),
        streamSettings=OutboundBean.StreamSettingsBean(),
    )
    return outbound
...
def generateConfig(config: str, dns_list=["8.8.8.8"]):
    ...
    if protocol == EConfigType.VLESS.protocolName:
        ...
        netquery = dict((k, v if len(v) > 1 else v[0]) for k, v in parse_qs(parsed_url.query).items())
        ...
        fingerprint = (
            netquery.get("fp")
            if "fp" in netquery
            else (streamSetting.tlsSettings.fingerprint if streamSetting.tlsSettings else None)
        )
